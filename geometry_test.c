...
{    coord_2d_t a;
   coord_2d_t b;
   coord_2d_t c;    a.x = a.y = b.x = c.y = 0;
   b.y = 4;
   c.x = 4;
   ck_assert(coord_2d_area_triangle(&a, &b, &c) == 8.0);    a.x = 0;
   a.y = 0;
   b.x = 3;
   b.y = 0;
   c.x = 3;
...
END_TEST/* coord_2d Test Suite */
Suite* coord_2d_suite(void)
{    /* Create Suite */
   Suite* s = suite_create("coord_2d");    /* Setup Test Cases */
   TCase* tc_2d_eq = tcase_create("coord_2d_eq");
   tcase_add_test(tc_2d_eq, test_2d_eq);    TCase* tc_2d_dist = tcase_create("coord_2d_dist");
   tcase_add_test(tc_2d_dist, test_2d_dist);    TCase* tc_2d_midpoint = tcase_create("coord_2d_midpoint");
   tcase_add_test(tc_2d_midpoint, test_2d_midpoint);    TCase* tc_2d_area_triangle = tcase_create("coord_2d_area_triangle");
   tcase_add_test(tc_2d_area_triangle, test_2d_area_triangle);    /* Add Cases to Suite */
